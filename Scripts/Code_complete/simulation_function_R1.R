...
run_ssb_sim <-
  function(sppEnv,
           ...
           env) {
    #--- Modelling section
    if (sum(sppEnv$pa) > 100) {
      # Don't run for very small range species
      
      #- Create SSB based on "feature" distance decay
      sppEnv$biasVar <- sppEnv$pathdd
      if (corNiche) {
        sppEnv$biasVar <- sppEnv$biasVar * scales::rescale(sppEnv$poc, c(0.1,0.9))
      }
      #ggplot(sppEnv) + geom_raster(aes(x = X, y = Y, fill = biasVar))
      samp_bias <-
        samp.bias.feature.dd(sppEnv[, c("id", "X", "Y", "biasVar")])
      
      # ggplot() +
      #   geom_raster(aes(x = X, y = Y, fill =  wgt1), data = samp_bias) +
      ...
      if (agrepl(ssb_meth, pattern = "spTgGrp")) {
        cor_wgh_df <-
          spTgGrp.corIntensity(sppEnv[, c("id", "X", "Y", "poc")])
      }
      ...
      rep_all <- lapply(trials, function(x) {
        ...
        spSamp <- species.sampling(
          sp_dat = sppEnv,
          bias = samp_bias[[trueBias]],
          # This get the true bias
          nPres = nPres,
          detProb = detProb,
          fpr = falPosRat,
        ...
        if (ssb_meth == "occFilter" & x != 0) {
          spDat <- filter.occurrences(sppEnv, spDat, as.numeric(x))
        }
        ...
        if (agrepl(ssb_meth, pattern = "spTgGrp")) {
          ...
          richness <- rpois(nrow(sppEnv), 20 * cor_wgh)
        ...
        possibleError <- tryCatch(
          spBkDat <-
            create.blocks(spBkDat, sppEnv, block.type, n_folds),
          error = function(e)
            e
        )
        ...
        if (!inherits(possibleError, "error")) {
          ...
          if (length(blocks) > 1) {
            for (blk in  blocks) {
              ...
              test.blk.ind <-
                sppEnv[sample(1:nrow(sppEnv), nrow(test.blk), replace = FALSE), ]
              
              #- Assign weightings
              p.wt <- rep(1.e-6, nrow(train.blk))
              p.wt[train.blk$det == 0] <-
                nrow(sppEnv) / sum(train.blk$det)
      ...
      rep_all$prevelance <- sum(sppEnv$pa)
      rep_all
    }
  }
...
prepare_sim_data <- function(nicheCor = FALSE) {
  ...
  f_sim_res <- f_sim_res[!grepl(f_sim_res, pattern = "_1600_|spTgGrp")]
  
  # f<- f_sim_res[grep(f_sim_res, pattern = "100_86_")]
  # f <- f[grep(f, pattern = "featDD")]
  # i <- f[56]
  
  f_sim_res_l <- lapply(f_sim_res, function(i) {
    print(i)
    possibleError <- tryCatch(
      sp_i  <- get(load(i)),
      error = function(e)
    ...
    if (inherits(possibleError, "error") | class(sp_i) == "list") {
      file.remove(i)
      return(NULL)
    } else {
